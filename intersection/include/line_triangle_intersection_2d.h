...
// line := (x1, y1) (x2, y2)
// triangle := (x1, y1) (x2, y2) (x3, y3)
bool IsIntersectedLineSegmentAndTriangle2D(const std::array<Vec2, 2> &line, const std::array<Vec2, 3> &triangle)
{
  ...
  return IsIntersectedLineSegment2D(line, edges[0]) ||
         IsIntersectedLineSegment2D(line, edges[1]) ||
         IsIntersectedLineSegment2D(line, edges[2]);
}
